function [J,varargout] = input_fun_SS_analytic_cons_slot_prop_v1(x,VAR)
...
lam1_Su = VAR.S_Su*lam1_S;
% lam1_S = zeros(length(x),1);
...
%     pen_sat_it_pos = diff_S(i,diff_S(i,:)>0);
    if ~isempty(pen_sat_it_neg)
        ...
        pen_const = pen_const + length(VAR.tspan)/i; % it penalizes the time step when intrinsic capacity is exceeded
    end
...
if ~isempty(check_S_Su_excess) 
    pen_unsl = VAR.beta3*sum(check_S_Su_excess);
else
    pen_unsl = 0;
end

%% force the increasing trend of launch rate

% diff_lam = diff(lam1_S);
% if diff_lam>0 
%     pen_lam = sum(lam1_S(diff_lam>0));
% else
%     pen_lam = 0;
% end
...
J = VAR.beta1/N_tot_real+pen_sat+pen_const+pen_time+pen_debris+pen_unsl;

if nargout==4
    varargout{1} = T;
    varargout{2} = X;
    varargout{3} = [pen_time pen_unsl pen_sat+pen_const pen_debris];
end
